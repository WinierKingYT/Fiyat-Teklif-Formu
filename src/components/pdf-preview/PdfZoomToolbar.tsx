...
const PdfZoomToolbar: React.FC<PdfZoomToolbarProps> = ({
    ...
    return (
        <div className="absolute bottom-4 right-4 z-20 flex items-center gap-1.5 px-2.5 py-1 bg-[var(--color-bg-card)]/90 backdrop-blur-md rounded-full shadow-lg border border-[var(--color-border)] text-xs">
            ...
            <button type="button" onClick={() => setZoomLevel(z => Math.max(0.3, Math.round((z - 0.1) * 10) / 10))} className="w-5 h-5 flex items-center justify-center rounded-full hover:bg-[var(--color-bg-hover)] text-[var(--color-text)] font-bold text-xs" aria-label={t('zoomOut')}>−</button>
            <span className="text-[11px] font-mono font-semibold text-[var(--color-text)] w-8 text-center">{Math.round(zoomLevel * 100)}%</span>
            <button type="button" onClick={() => setZoomLevel(z => Math.min(2, Math.round((z + 0.1) * 10) / 10))} className="w-5 h-5 flex items-center justify-center rounded-full hover:bg-[var(--color-bg-hover)] text-[var(--color-text)] font-bold text-xs" aria-label={t('zoomIn')}>+</button>
