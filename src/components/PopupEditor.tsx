    ...
    useEffect(() => {
        if (isOpen && inputRef.current) {
            setTimeout(() => {
                ...
                if (el) {
                    el.focus();
                    if (type !== 'select') (el as HTMLInputElement | HTMLTextAreaElement).select();
                }
            }, 100);
        }
    }, [isOpen, type]);
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
            <div role="dialog" aria-modal="true" aria-label={title} className="bg-[var(--color-bg-card)] rounded-[var(--radius-lg)] shadow-lg w-full max-w-md border border-[var(--color-border)] animate-scaleIn">
                ...
                <form onSubmit={handleSubmit} className="p-5">
                    <div className="mb-5">
                        {type === 'textarea' ? (
                            <textarea ref={(el) => { inputRef.current = el; }} value={value} onChange={(e) => setValue(e.target.value)} className={inputClass + " min-h-[120px] resize-y"} placeholder="Değer giriniz..." />
                        ) : type === 'select' ? (
                            <select ref={(el) => { inputRef.current = el; }} value={value} onChange={(e) => setValue(e.target.value)} className={inputClass}>
                                {options.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                            </select>
                        ) : (
                            <input ref={(el) => { inputRef.current = el; }} type={type} value={value} onChange={(e) => setValue(e.target.value)} className={inputClass} placeholder="Değer giriniz..." />
                        )}
