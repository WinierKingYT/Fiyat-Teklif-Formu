...
const RecycleBinModal = ({ isOpen, onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState('all'); // 'all', 'customers', 'products'
    ...
    const handleRestore = async (item) => {
        try {
            ...
            // We remove recycle bin specific fields
            const { id, originalStore, deletedAt, originalId, ...originalData } = item;

            // Use originalId if possible, but handle conflicts if ID exists?
            // IndexedDB add/put will handle it. If we use put with originalId, it overwrites.
            // If we use add, it might fail if key exists.
            // Let's use put to ensure it restores.
            ...
            <div className="flex flex-col h-[70vh]">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <div className="flex gap-2">
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'all' ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg-muted)] text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'}`}
                            onClick={() => setActiveTab('all')}
                        >
                            Tümü
                        </button>
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'customers' ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg-muted)] text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'}`}
                            onClick={() => setActiveTab('customers')}
                        >
                            Müşteriler
                        </button>
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'products' ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg-muted)] text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'}`}
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-[var(--color-bg-muted)] rounded-lg border border-[var(--color-border)] p-2">
                    {filteredItems.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {filteredItems.map(item => (
                                <div key={item.id} className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-[var(--color-border)] flex justify-between items-center hover:shadow-sm transition-shadow">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-xs px-2 py-0.5 rounded-full ${item.originalStore === 'customers' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' : 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300'}`}>
                                        ...
                                        <div className="font-medium text-[var(--color-text)]">
                                            {item.name || item.company || 'İsimsiz Öğe'}
                                        </div>
                                        {item.originalStore === 'products' && (
                                            <div className="text-sm text-[var(--color-text-muted)]">
                                                {item.price} ₺
                                            </div>
                                        )}
                                        {item.originalStore === 'customers' && (
                                            <div className="text-sm text-[var(--color-text-muted)]">
                                                {item.company}
                                            </div>
                                        )}
