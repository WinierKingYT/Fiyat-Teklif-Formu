...
const PerformanceMaintenanceTab = () => {
    ...
    if (loading) {
        return <div className="p-8 text-center dark:text-gray-300">Yükleniyor...</div>;
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-semibold dark:text-white">Performans & Bakım</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Uygulama performansını izleyin ve bakım işlemlerini yönetin</p>
                </div>
                <button
                ...
                <div className="p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            <h4 className="font-semibold text-orange-900 dark:text-orange-200 mb-2">Performans Uyarıları</h4>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Memory Usage */}
                    {metrics?.memory && (
                        <div className="p-4 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-sm font-medium dark:text-gray-300">Bellek Kullanımı</span>
                                <span className="text-xs text-gray-500 dark:text-gray-400">{metrics.memory.usedMB} / {metrics.memory.limitMB} MB</span>
                            </div>
                            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                <div
                                    className={`h-2 rounded-full transition-all ${parseFloat(metrics.memory.usedPercentage) > 80 ? 'bg-red-600' : parseFloat(metrics.memory.usedPercentage) > 60 ? 'bg-orange-500' : 'bg-green-500'}`}
                                    style={{ width: `${metrics.memory.usedPercentage}%` }}
                                />
                            </div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">%{metrics.memory.usedPercentage} kullanımda</p>
                        </div>
                    )}
                    ...
                    {metrics?.storage && (
                        <div className="p-4 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-sm font-medium dark:text-gray-300">Depolama Alanı</span>
                                <span className="text-xs text-gray-500 dark:text-gray-400">{metrics.storage.usageMB} / {metrics.storage.quotaMB} MB</span>
                            </div>
                            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                <div
                                    className={`h-2 rounded-full transition-all ${parseFloat(metrics.storage.usagePercentage) > 85 ? 'bg-red-600' : parseFloat(metrics.storage.usagePercentage) > 70 ? 'bg-orange-500' : 'bg-blue-500'}`}
                                    style={{ width: `${metrics.storage.usagePercentage}%` }}
                                />
                            </div>
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">%{metrics.storage.usagePercentage} kullanımda</p>
                        </div>
                    )}
                    ...
                    {metrics?.indexedDB && (
                        <div className="p-4 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-sm font-medium dark:text-gray-300">IndexedDB Boyutu</span>
                                <span className="text-xs text-gray-500 dark:text-gray-400">{metrics.indexedDB.totalMB} MB</span>
                            </div>
                            <div className="space-y-1 mt-2 text-xs">
                                {Object.entries(metrics.indexedDB.stores).map(([name, info]) => (
                                    <div key={name} className="flex justify-between text-gray-600 dark:text-gray-400">
                                        <span>{name}</span>
                    ...
                    {metrics?.sessionInfo && (
                        <div className="p-4 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-sm font-medium dark:text-gray-300">Oturum Bilgileri</span>
                            </div>
            ...
            {/* Cleanup Settings */}
            <div>
                <h4 className="font-semibold mb-3 dark:text-white">Otomatik Temizleme Ayarları</h4>

                <div className="space-y-4">
                    {/* Enable Auto Cleanup */}
                    <label className="flex items-center gap-3 p-3 border rounded-lg dark:border-slate-700 dark:bg-slate-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700">
                        <input
                            type="checkbox"
                        ...
                        />
                        <div className="flex-1">
                            <span className="font-medium dark:text-gray-200">Otomatik Temizlemeyi Etkinleştir</span>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Belirlenen sürelere göre eski verileri otomatik temizle</p>
                        </div>
                    </label>
                    ...
                    {cleanupSettings.autoCleanupEnabled && (
                        <label className="flex items-center gap-3 p-3 border rounded-lg dark:border-slate-700 dark:bg-slate-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700">
                            ...
                            />
                            <div className="flex-1">
                                <span className="font-medium dark:text-gray-200">Başlangıçta Temizle</span>
                                <p className="text-xs text-gray-500 dark:text-gray-400">Uygulama her açıldığında otomatik temizlik yap</p>
                            </div>
                        </label>
                    )}
                    ...
                    {/* Retention Periods */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-3 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            ...
                            />
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Bu süreden eski teklifler temizlenebilir</p>
                        </div>
                        ...
                        <div className="p-3 border rounded-lg dark:border-slate-700 dark:bg-slate-800">
                            ...
                            <input
                                ...
                                className="form-control"
                            />
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Bu süreden eski silinmiş öğeler temizlenebilir</p>
                        </div>
                    </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <button
                        ...
                        className="p-4 border rounded-lg text-left hover:bg-gray-50 dark:hover:bg-slate-700 dark:border-slate-700 dark:bg-slate-800 transition-colors"
                        disabled={!cleanupStats || cleanupStats.oldQuotes === 0}
                    >
                        <div className="flex items-center gap-3">
                            <Trash2 size={20} className="text-orange-600 dark:text-orange-400" />
                            <div>
                                <div className="font-medium dark:text-gray-200">Eski Teklifleri Temizle</div>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <Trash2 size={20} className="text-purple-600 dark:text-purple-400" />
                            <div>
                                <div className="font-medium dark:text-gray-200">Geri Dönüşüm Kutusunu Boşalt</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-medium dark:text-gray-200">Bağlantısız Verileri Temizle</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-medium dark:text-gray-200">Geçmişi Sınırla</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400">
                ...
                <div className="mt-4 pt-4 border-t dark:border-slate-700">
                    <button
                        ...
                        className="w-full p-4 border-2 border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-900/20 rounded-lg text-left hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
                    >
                        <div className="flex items-center gap-3">
                            <AlertTriangle size={24} className="text-red-600 dark:text-red-400" />
                            <div className="flex-1">
                                <div className="font-bold text-red-900 dark:text-red-200">TAM TEMİZLİK YAP</div>
