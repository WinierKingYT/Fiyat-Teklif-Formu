import React from 'react';
import { translations } from '../utils/translations';

// Helper functions defined outside component to avoid recreation


const formatDate = (dateString, locale = 'tr-TR') => {
    if (!dateString) return '-';
    return new Date(dateString).toLocaleDateString(locale);
};

const PrintableQuote = ({
    id,
    theme = 'modern',
    color = '#000000',
    quoteData: _quoteData,
    customerData: _customerData,
    companyData: _companyData,
    bankData: _bankData,
    items: _items,
    discount: _discount,
    config: _config,
    layout,
    signature
}) => {
    const quoteData = React.useMemo(() => _quoteData || {}, [_quoteData]);
    const customerData = React.useMemo(() => _customerData || {}, [_customerData]);
    const companyData = React.useMemo(() => _companyData || {}, [_companyData]);
    const bankData = React.useMemo(() => _bankData || {}, [_bankData]);
    const items = React.useMemo(() => _items || [], [_items]);
    const discount = React.useMemo(() => _discount || {}, [_discount]);
    const hasLineItemDiscounts = React.useMemo(() => items.some(item => (item.discountRate || 0) > 0), [items]);

    const language = quoteData.language || 'tr';
    const t = translations[language] || translations['tr'];

    const localeMap = {
        'tr': 'tr-TR',
        'en': 'en-US',
        'de': 'de-DE'
    };
    const currentLocale = localeMap[language] || 'tr-TR';

    const formatCurrency = React.useCallback((amount) => {
        const currency = quoteData.currency || 'TRY';
        return new Intl.NumberFormat(currentLocale, { style: 'currency', currency: currency }).format(amount);
    }, [quoteData.currency, currentLocale]);

    // Default configuration
    const config = React.useMemo(() => ({
        showLogo: true,
        showBankInfo: true,
        showSignatures: true,
        showTerms: true,
        showNotes: true,
        showSummary: true,
        title: 'FİYAT TEKLİFİ',
        fontFamily: 'Inter',
        fontSize: 'medium',
        margins: 'normal',
        showTableImages: false,
        showTableUnit: true,
        showTableTax: true,
        showQRCode: false,
        qrCodeUrl: '',
        showWatermark: false,
        watermarkText: 'TASLAK',
        watermarkOpacity: 0.1,
        watermarkColor: '#000000',
        watermarkFontSize: 120,
        watermarkRotation: -45,
        customFooter: '',
        logoPosition: 'left',
        ...(_config || {})
    }), [_config]);

    const { subtotal, totalTax, total, discountAmount, taxBreakdown } = React.useMemo(() => {
        let subtotal = 0;
        let rawTotalTax = 0;
        const rawTaxBreakdown = {};

        items.forEach(item => {
            const total = (item.quantity || 0) * (item.price || 0);
            subtotal += total;
            const tax = total * ((item.taxRate || 0) / 100);
            rawTotalTax += tax;

            const rate = item.taxRate || 0;
            if (tax > 0) {
                rawTaxBreakdown[rate] = (rawTaxBreakdown[rate] || 0) + tax;
            }
        });

        let discountAmount = 0;
        let discountRatio = 0;

        if (subtotal > 0) {
            if (discount?.type === 'percentage') {
                discountAmount = subtotal * ((discount.value || 0) / 100);
                discountRatio = (discount.value || 0) / 100;
            } else if (discount?.type === 'fixed') {
                discountAmount = Number(discount.value) || 0;
                discountRatio = discountAmount / subtotal;
            }
        }

        // Ensure discount doesn't exceed subtotal
        if (discountAmount > subtotal) {
            discountAmount = subtotal;
            discountRatio = 1;
        }

        // Recalculate tax based on discounted amount
        const totalTax = rawTotalTax * (1 - discountRatio);

        const taxBreakdown = {};
        Object.keys(rawTaxBreakdown).forEach(rate => {
            taxBreakdown[rate] = rawTaxBreakdown[rate] * (1 - discountRatio);
        });

        const total = subtotal - discountAmount + totalTax;

        return { subtotal, totalTax, total, discountAmount, taxBreakdown };
    }, [items, discount]);

    const modernStyles = React.useMemo(() => `
        .modern-theme-container {
            font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;
            line-height: 1.4;
            color: #1e293b;
            background: white;
            font-size: ${config.fontSize || 12}px;
            position: relative;
            overflow: hidden;
            border-radius: ${config.borderRadius || 6}px;
            box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);
        }

        .modern-theme-container::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
            height: 3px;
            background: linear-gradient(90deg, ${color}, #ededed);
        }

        .modern-theme-container, .modern-theme-container * {
            box-sizing: border-box;
        }
        
        .modern-theme-container .pdf-header {
            display: flex;
            justify-content: space-between;
            align-items: ${config.logoPosition === 'center' ? 'center' : 'flex-start'};
            flex-direction: ${config.logoPosition === 'center' ? 'column' : 'row'};
            margin-bottom: 1rem;
            padding-bottom: 1rem;
            border-bottom: 1px solid #e2e8f0;
            text-align: ${config.logoPosition === 'center' ? 'center' : 'left'};
        }
        
        .modern-theme-container .header-left {
            display: flex;
            align-items: ${config.logoPosition === 'center' ? 'center' : 'flex-start'};
            flex-direction: ${config.logoPosition === 'center' ? 'column' : config.logoPosition === 'right' ? 'row-reverse' : 'row'};
            gap: 1.5rem;
            flex: 1;
            width: ${config.logoPosition === 'center' ? '100%' : 'auto'};
            text-align: ${config.logoPosition === 'right' ? 'right' : config.logoPosition === 'center' ? 'center' : 'left'};
        }
        
        .modern-theme-container .company-logo {
            width: 150px;
            height: 75px;
            border: 1px solid #e2e8f0;
            border-radius: ${config.borderRadius || 6}px;
            display: flex;
            align-items: center;
            justify-content: center;
            background: #f8fafc;
            color: ${color};
            font-size: 0.6rem;
            text-align: center;
            padding: 0.5rem;
            overflow: hidden;
        }
        
        .modern-theme-container .company-logo img {
            max-width: 100%;
            max-height: 100%;
            object-fit: contain;
        }
        
        .modern-theme-container .company-info {
            flex: 1;
        }
        
        .modern-theme-container .company-name {
            font-size: 1rem;
            font-weight: 700;
            color: ${color};
            margin-bottom: 0.25rem;
        }
        
        .modern-theme-container .company-details {
            font-size: 0.7rem;
            color: #000000;
            line-height: 1.3;
        }

        .modern-theme-container .header-info-grid {
            display: grid;
            gap: 0.125rem;
        }

        .modern-theme-container .header-info-line {
            display: flex;
        }

        .modern-theme-container .header-info-label {
            font-weight: 600;
            min-width: 60px;
            color: #475569;
        }
        
        .modern-theme-container .quote-info {
            text-align: ${config.logoPosition === 'right' ? 'left' : config.logoPosition === 'center' ? 'center' : 'right'};
            background: #f8fafc;
            padding: 0.8rem;
            border-radius: ${config.borderRadius || 6}px;
            border-left: ${config.logoPosition === 'right' ? 'none' : `3px solid ${color}`};
            border-right: ${config.logoPosition === 'right' ? `3px solid ${color}` : 'none'};
            border-top: ${config.logoPosition === 'center' ? `3px solid ${color}` : 'none'};
            min-width: 200px;
            margin-top: ${config.logoPosition === 'center' ? '1rem' : '0'};
            width: ${config.logoPosition === 'center' ? '100%' : 'auto'};
        }
        
        .modern-theme-container .quote-title {
            font-size: 0.9rem;
            font-weight: 700;
            color: ${color};
            margin-bottom: 0.5rem;
        }
        
        .modern-theme-container .quote-meta {
            font-size: 0.7rem;
            color: #64748b;
        }
        
        .modern-theme-container .quote-meta div {
            margin-bottom: 0.125rem;
        }
        
        .modern-theme-container .quote-number {
            font-weight: 600;
            color: #000000;
        }
        
        .modern-theme-container .customer-section {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
            margin-bottom: 1rem;
        }
        
        .modern-theme-container .customer-box {
            background: #f8fafc;
            border-radius: ${config.borderRadius || 6}px;
            padding: 0.8rem;
            border: 1px solid #e2e8f0;
        }
        
        .modern-theme-container .section-title {
            font-size: 0.8rem;
            font-weight: 600;
            color: ${color};
            margin-bottom: 0.5rem;
            display: flex;
            align-items: center;
            gap: 0.375rem;
        }
        
        .modern-theme-container .section-title i {
            color: ${color};
            font-size: 0.7rem;
        }
        
        .modern-theme-container .info-grid {
            display: grid;
            gap: 0.3rem;
        }
        
        .modern-theme-container .info-line {
            display: flex;
            font-size: 0.7rem;
        }
        
        .modern-theme-container .info-label {
            font-weight: 500;
            color: #475569;
            min-width: 60px;
        }
        
        .modern-theme-container .info-value {
            color: #1e293b;
            flex: 1;
        }
        
        .modern-theme-container .pdf-items-table {
            width: 100%;
            border-collapse: collapse;
            margin: 1rem 0;
            font-size: 0.7rem;
            background: white;
            border-radius: ${config.borderRadius || 6}px;
            letter-spacing: 0.05em;
            border-bottom: 1px solid #e2e8f0;
        }
        
        .modern-theme-container .pdf-items-table th {
            background: ${config.tableHeaderBg || '#f1f5f9'};
            padding: 4px !important;
            height: auto !important;
            min-height: ${config.tableRowHeight || 35}px;
            vertical-align: middle;
            text-align: center;
            font-weight: 700;
            color: ${color};
            font-size: ${config.tableHeaderFontSize || 14}px !important;
            line-height: 1.2;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            border-bottom: 1px solid #e2e8f0;
        }

        .pdf-items-table tr {
            page-break-inside: avoid;
            page-break-after: auto;
        }

        .pdf-items-table td img {
            max-width: 100%;
            height: auto;
            display: block;
        }
        
        .modern-theme-container .pdf-items-table td {
            padding: 0.4rem 0.3rem;
            border-bottom: 1px solid #f1f5f9;
            vertical-align: middle;
        }
        
        .modern-theme-container .pdf-items-table tr:last-child td {
            border-bottom: none;
        }
        
        .modern-theme-container .pdf-items-table tr:nth-child(even) {
            background: #f8fafc;
        }

        .modern-theme-container .pdf-items-table tr:hover {
            background: #f1f5f9;
        }
        
        .modern-theme-container .item-image {
            width: 40px;
            height: 40px;
            border-radius: ${Math.max((config.borderRadius || 6) - 2, 2)}px;
            object-fit: cover;
            background: #f1f5f9;
            display: flex;
            align-items: center;
            justify-content: center;
            color: #94a3b8;
            font-size: 0.5rem;
            text-align: center;
            overflow: hidden;
        }
        
        .modern-theme-container .item-image img {
            width: 100%;
            height: 100%;
            object-fit: cover;
            border-radius: 4px;
        }

        .modern-theme-container .item-description {
            font-weight: 500;
            color: #1e293b;
        }
        
        .modern-theme-container .item-details {
            font-size: 0.6rem;
            color: #64748b;
            margin-top: 0.2rem;
            line-height: 1.2;
        }
        
        .modern-theme-container .item-price, 
        .modern-theme-container .item-total {
            font-weight: 500;
            color: #1e293b;
            text-align: center;
        }
        
        .modern-theme-container .item-quantity, 
        .modern-theme-container .item-tax, 
        .modern-theme-container .item-unit {
            color: #64748b;
            text-align: center;
        }
        
        .modern-theme-container .pdf-summary {
            margin: 1rem 0;
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
        }
        
        .modern-theme-container .summary-box {
            background: white;
            border-radius: ${config.borderRadius || 6}px;
            padding: 0.8rem;
            border: 1px solid #e2e8f0;
        }
        
        .modern-theme-container .summary-row {
            display: flex;
            justify-content: space-between;
            padding: 0.3rem 0;
            border-bottom: 1px solid #f1f5f9;
            font-size: 0.7rem;
        }
        
        .modern-theme-container .summary-row:last-child {
            border-bottom: none;
        }
        
        .modern-theme-container .summary-row.grand-total {
            font-weight: 700;
            font-size: 0.8rem;
            color: ${color};
            padding-top: 0.5rem;
            margin-top: 0.25rem;
            border-top: 2px solid #e2e8f0;
        }
        
        .modern-theme-container .summary-title {
            font-size: 0.8rem;
            font-weight: 600;
            color: ${color};
            margin-bottom: 0.5rem;
            display: flex;
            align-items: center;
            gap: 0.5rem;
        }

        .modern-theme-container .payment-info h3 {
            font-size: 0.8rem;
            font-weight: 600;
            color: ${color};
            margin-bottom: 0.5rem;
        }
        
        .modern-theme-container .signature-section {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
            margin: 1rem 0;
            padding-top: 1rem;
            border-top: 1px solid #e2e8f0;
        }
        
        .modern-theme-container .signature-box {
            text-align: center;
            padding: 0.8rem;
        }
        
        .modern-theme-container .signature-area {
            height: 60px;
            border-bottom: 1px solid #cbd5e1;
            margin-bottom: 0.5rem;
            display: flex;
            align-items: flex-end;
            justify-content: center;
            position: relative;
        }
        
        .modern-theme-container .stamp-area {
            height: 60px;
            border: 1px dashed #cbd5e1;
            border-radius: 4px;
            display: flex;
            align-items: center;
            justify-content: center;
            color: #94a3b8;
            font-size: 0.6rem;
            margin-bottom: 0.5rem;
            background: #f8fafc;
            overflow: hidden;
        }
        
        .modern-theme-container .signature-label {
            font-size: 0.7rem;
            color: #64748b;
            font-weight: 500;
        }
        
        .modern-theme-container .terms-section {
            margin: 1rem 0;
        }
        
        .modern-theme-container .terms-grid {
            display: grid;
            grid-template-columns: 1fr 1fr;
        .modern-theme-container .pdf-footer {
            margin-top: 1rem;
            padding-top: 1rem;
            border-top: 1px solid #e2e8f0;
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
            font-size: 0.7rem;
            color: #64748b;
        }
        
        .modern-theme-container .footer-contact div {
            margin-bottom: 0.125rem;
        }
        
        .modern-theme-container .footer-thanks {
            text-align: right;
        }
        
        .modern-theme-container .footer-thanks .thanks-text {
            font-weight: 500;
            color: ${color};
            margin-bottom: 0.25rem;
        }

        /* PDF Optimizasyon Stilleri */
        .pdf-compact-mode .pdf-items-table {
            font-size: 6px !important;
        }

        .pdf-compact-mode .pdf-items-table th {
            padding: 3px 2px !important;
            line-height: 1.1 !important;
            font-size: 8px !important;
            text-align: center !important;
            height: 35px !important;
            vertical-align: middle !important;
        }

        .pdf-compact-mode .pdf-items-table td {
            padding: 2px 1px !important;
            line-height: 1 !important;
            vertical-align: middle !important;
        }

        .pdf-compact-mode .item-price,
        .pdf-compact-mode .item-total,
        .pdf-compact-mode .item-quantity,
        .pdf-compact-mode .item-tax,
        .pdf-compact-mode .item-unit {
            text-align: center !important;
        }

        .pdf-compact-mode .item-image {
            width: 25px !important;
            height: 25px !important;
        }

        .pdf-compact-mode .section-title {
            font-size: 7px !important;
            margin-bottom: 3px !important;
        }

        .pdf-compact-mode body {
            font-size: 8px !important;
            line-height: 1.1 !important;
        }

        /* Ultra kompakt mod */
        .pdf-ultra-compact .pdf-items-table {
            font-size: 5px !important;
        }

        .pdf-ultra-compact .pdf-items-table td,
        .pdf-ultra-compact .pdf-items-table th {
            padding: 1px 0.5px !important;
            line-height: 0.9 !important;
        }

        .pdf-ultra-compact .item-image {
            width: 20px !important;
            height: 20px !important;
        }

        /* Sayfa sonu optimizasyonu */
        .page-break-avoid {
            page-break-inside: avoid;
        }

        .keep-together {
            page-break-inside: avoid;
        }

        /* PDF Sayfa Düzeni Optimizasyonları */
        .pdf-items-table-compact {
            font-size: 9px !important;
            line-height: 1.1 !important;
        }

        .pdf-items-table-compact th,
        .pdf-items-table-compact td {
            padding: 3px 2px !important;
            line-height: 1.1 !important;
        }

        .pdf-items-table-compact th {
            font-size: 8px !important;
            padding: 4px 2px !important;
        }

        .item-image-compact {
            width: 25px !important;
            height: 25px !important;
            min-width: 25px !important;
            min-height: 25px !important;
        }

        .item-image-compact img {
            width: 100% !important;
            height: 100% !important;
            object-fit: cover !important;
        }

        /* Tablo başlığını her sayfada tekrarla */
        .pdf-items-table thead {
            display: table-header-group;
        }

        .pdf-items-table tbody {
            display: table-row-group;
        }

        /* Özet bölümü için sayfa sonu optimizasyonu */
        .pdf-summary {
            page-break-inside: avoid;
        }

        /* İmza bölümü için sayfa sonu optimizasyonu */
        .signature-section {
            page-break-inside: avoid;
            margin-top: 20px;
        }
    `, [config, color]);

    const minimalStyles = React.useMemo(() => `
        .minimal-theme-container {
            font-family: 'Inter', sans-serif;
            line-height: 1.5;
            color: #333;
            background: white;
            font-size: ${config.fontSize || 12}px;
            position: relative;
        }
        
        .minimal-theme-container .pdf-header {
            display: flex;
            justify-content: space-between;
            align-items: flex-start;
            margin-bottom: 4rem;
        }

        .minimal-theme-container .company-logo {
            margin-bottom: 1.5rem;
        }

        .minimal-theme-container .company-logo img {
            max-height: 60px;
            object-fit: contain;
        }

        .minimal-theme-container .company-name {
            font-size: 1.1rem;
            font-weight: 700;
            color: #000;
            margin-bottom: 0.25rem;
            letter-spacing: -0.01em;
        }

        .minimal-theme-container .company-details {
            font-size: 0.75rem;
            color: #666;
            line-height: 1.5;
        }

        .minimal-theme-container .quote-title {
            font-size: 2.5rem;
            font-weight: 300;
            letter-spacing: -0.03em;
            color: #000;
            text-align: right;
            margin-bottom: 1rem;
        }

        .minimal-theme-container .quote-meta {
            text-align: right;
            font-size: 0.75rem;
            color: #666;
            margin-top: 0.5rem;
        }
        
        .minimal-theme-container .quote-meta div {
            margin-bottom: 0.25rem;
        }

        .minimal-theme-container .customer-section {
            margin-bottom: 4rem;
            padding-left: 1rem;
            border-left: 2px solid #000;
        }

        .minimal-theme-container .customer-label {
            font-size: 0.65rem;
            text-transform: uppercase;
            letter-spacing: 0.1em;
            color: #999;
            margin-bottom: 0.5rem;
            font-weight: 600;
        }

        .minimal-theme-container .customer-name {
            font-size: 1.4rem;
            font-weight: 600;
            color: #000;
            margin-bottom: 0.25rem;
            letter-spacing: -0.02em;
        }

        .minimal-theme-container .customer-details {
            font-size: 0.85rem;
            color: #555;
        }

        .minimal-theme-container table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 3rem;
        }

        .minimal-theme-container th {
            text-align: left;
            font-weight: 600;
            color: #999;
            padding: 1rem;
            font-size: 0.7rem;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            border-bottom: 1px solid #eee;
        }

        .minimal-theme-container td {
            padding: 1rem;
            color: #333;
            border: none;
        }
        
        .minimal-theme-container tr:nth-child(even) {
            background-color: #f9fafb;
        }

        .minimal-theme-container .total-section {
            display: flex;
            justify-content: flex-end;
            margin-top: 3rem;
        }

        .minimal-theme-container .total-box {
            width: 280px;
        }

        .minimal-theme-container .total-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.75rem;
            font-size: 0.85rem;
            color: #666;
        }

        .minimal-theme-container .total-row.final {
            margin-top: 1.5rem;
            padding-top: 1.5rem;
            border-top: 1px solid #eee;
            font-size: 1.5rem;
            font-weight: 300;
            color: #000;
            letter-spacing: -0.02em;
            align-items: baseline;
        }
        
        .minimal-theme-container .footer {
            margin-top: 5rem;
            padding-top: 2rem;
            border-top: 1px solid #f3f4f6;
            display: flex;
            justify-content: space-between;
            font-size: 0.7rem;
            color: #9ca3af;
        }
    `, [config]);

    const corporateStyles = React.useMemo(() => `
        .corporate-theme-container {
            font-family: 'Roboto', sans-serif;
            line-height: 1.5;
            color: #1a1a1a;
            background: white;
            font-size: ${config.fontSize || 12}px;
            position: relative;
        }

        .corporate-theme-container .header-bar {
            background: ${color};
            color: white;
            padding: 3rem 2rem;
            display: flex;
            justify-content: space-between;
            align-items: flex-start;
            margin-bottom: 3rem;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }

        .corporate-theme-container .company-name {
            font-size: 2rem;
            font-weight: 700;
            letter-spacing: -0.5px;
            margin-bottom: 0.5rem;
        }

        .corporate-theme-container .company-details {
            font-size: 0.85rem;
            opacity: 0.9;
            line-height: 1.4;
        }

        .corporate-theme-container .quote-title {
            font-size: 1.5rem;
            font-weight: 700;
            margin-bottom: 0.5rem;
            text-align: right;
            text-transform: uppercase;
            letter-spacing: 0.05em;
        }

        .corporate-theme-container .quote-meta {
            font-size: 0.85rem;
            opacity: 0.95;
            text-align: right;
            line-height: 1.4;
        }

        .corporate-theme-container .content-wrapper {
            padding: 0 2rem;
        }

        .corporate-theme-container .info-grid {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 2rem;
            margin-bottom: 3rem;
        }

        .corporate-theme-container .info-box {
            background: #f8f9fa;
            border: 1px solid #e9ecef;
            padding: 1.5rem;
            border-radius: 0;
            border-left: 4px solid ${color};
        }

        .corporate-theme-container .box-title {
            font-size: 0.75rem;
            font-weight: 700;
            color: #495057;
            margin-bottom: 1rem;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            display: flex;
            align-items: center;
            gap: 0.5rem;
        }

        .corporate-theme-container .box-content {
            font-size: 0.9rem;
            color: #212529;
        }
        
        .corporate-theme-container .info-line {
            display: flex;
            margin-bottom: 0.25rem;
        }
        
        .corporate-theme-container .info-label {
            width: 80px;
            font-weight: 500;
            color: #6c757d;
        }

        .corporate-theme-container table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 3rem;
        }

        .corporate-theme-container th {
            background: ${color};
            color: white;
            font-weight: 600;
            text-align: left;
            padding: 1rem;
            font-size: 0.8rem;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }

        .corporate-theme-container td {
            padding: 1rem;
            border-bottom: 1px solid #dee2e6;
            color: #212529;
        }

        .corporate-theme-container tr:nth-child(even) {
            background-color: #f8f9fa;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }

        .corporate-theme-container .summary-section {
            display: flex;
            justify-content: flex-end;
            margin-top: 2rem;
        }

        .corporate-theme-container .summary-box {
            background: #f8f9fa;
            padding: 2rem;
            border: 1px solid #e9ecef;
            width: 350px;
            border-right: 4px solid ${color};
        }

        .corporate-theme-container .summary-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.75rem;
            font-size: 0.9rem;
            color: #495057;
        }

        .corporate-theme-container .grand-total {
            margin-top: 1.5rem;
            padding-top: 1.5rem;
            border-top: 2px solid #dee2e6;
            font-weight: 700;
            font-size: 1.4rem;
            color: ${color};
            display: flex;
            justify-content: space-between;
            align-items: center;
        }

        .corporate-theme-container .footer {
            margin-top: 5rem;
            padding: 2rem;
            background: #343a40;
            color: white;
            text-align: center;
            font-size: 0.8rem;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }
    `, [config, color]);

    const classicStyles = React.useMemo(() => `
        .classic-theme-container {
            font-family: 'Playfair Display', serif;
            line-height: 1.6;
            color: #1a1a1a;
            background: white;
            font-size: ${config.fontSize || 12}px;
            position: relative;
            padding: 3rem !important;
            border: 1px solid #000;
            outline: 4px double #000;
            outline-offset: -10px;
            margin: 0 auto;
            min-height: 297mm;
        }

        .classic-theme-container .pdf-header {
            text-align: center;
            margin-bottom: 3rem;
            padding-bottom: 2rem;
            position: relative;
            border-bottom: 1px solid #000;
        }

        .classic-theme-container .pdf-header::after {
            content: '';
            position: absolute;
            bottom: 2px;
            left: 20%;
            right: 20%;
            border-bottom: 1px solid #000;
        }

        .classic-theme-container .company-logo {
            margin-bottom: 1.5rem;
            display: flex;
            justify-content: center;
        }

        .classic-theme-container .company-logo img {
            max-height: 90px;
            object-fit: contain;
        }

        .classic-theme-container .company-name {
            font-size: 2.4rem;
            font-weight: 700;
            margin-bottom: 0.5rem;
            letter-spacing: 0.1em;
            text-transform: uppercase;
            font-variant: small-caps;
        }

        .classic-theme-container .company-info {
            font-size: 0.9rem;
            color: #444;
            font-style: italic;
            font-family: 'Times New Roman', serif;
        }

        .classic-theme-container .quote-title {
            font-size: 2rem;
            font-weight: 700;
            text-align: center;
            margin: 2.5rem 0;
            text-transform: uppercase;
            letter-spacing: 0.2em;
            color: #000;
        }

        .classic-theme-container .info-section {
            display: flex;
            justify-content: space-between;
            margin-bottom: 3rem;
            padding: 0 1rem;
        }

        .classic-theme-container .info-block {
            flex: 1;
        }

        .classic-theme-container .info-block.right {
            text-align: right;
        }

        .classic-theme-container .info-label {
            font-weight: 700;
            font-size: 0.85rem;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            border-bottom: 1px solid #000;
            padding-bottom: 2px;
            margin-bottom: 5px;
            display: inline-block;
        }

        .classic-theme-container .info-value {
            margin-bottom: 0.25rem;
        }

        .classic-theme-container table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 2rem;
            border-top: 2px solid #000;
            border-bottom: 2px solid #000;
        }

        .classic-theme-container th {
            text-align: left;
            padding: 1rem 0.5rem;
            font-weight: 700;
            text-transform: uppercase;
            font-size: 0.85rem;
            letter-spacing: 0.05em;
            border-bottom: 1px solid #000;
        }

        .classic-theme-container td {
            padding: 1rem 0.5rem;
            border-bottom: 1px solid #eee;
        }

        .classic-theme-container .total-section {
            display: flex;
            justify-content: flex-end;
            margin-top: 2rem;
        }

        .classic-theme-container .total-box {
            width: 300px;
            border: 1px solid #000;
            padding: 1.5rem;
        }

        .classic-theme-container .total-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.5rem;
            font-size: 0.9rem;
        }

        .classic-theme-container .total-row.final {
            margin-top: 1rem;
            padding-top: 1rem;
            border-top: 1px solid #000;
            font-weight: 700;
            font-size: 1.2rem;
        }

        .classic-theme-container .signature-section {
            margin-top: 4rem;
            display: flex;
            justify-content: space-between;
            padding: 0 2rem;
        }

        .classic-theme-container .signature-box {
            width: 200px;
            text-align: center;
        }

        .classic-theme-container .signature-line {
            border-bottom: 1px solid #000;
            height: 50px;
            margin-bottom: 0.5rem;
        }

        .classic-theme-container .signature-label {
            font-size: 0.8rem;
            font-style: italic;
        }

        .classic-theme-container .footer {
            text-align: center;
            margin-top: 4rem;
            font-size: 0.8rem;
            border-top: 1px solid #000;
            padding-top: 1rem;
            font-style: italic;
        }
    `, [config]);

    const getContainerStyles = () => {
        const baseStyles = {
            minHeight: 'auto',
            fontFamily: config.fontFamily === 'Playfair Display' ? "'Playfair Display', serif" :
                config.fontFamily === 'Roboto' ? "'Roboto', sans-serif" :
                    "'Inter', sans-serif",
            fontSize: `${config.fontSize || 12}px`
        };

        // Margins (Padding)
        const marginMap = {
            compact: '1.5rem', // ~6mm
            normal: '1.5rem',  // ~10mm (Adjusted to match preview.html which uses 1.5rem)
            wide: '4rem'       // ~16mm
        };
        const padding = marginMap[config.margins] || '1.5rem';

        return { ...baseStyles, padding };
    };

    const containerStyles = getContainerStyles();







    // Default layout if not provided
    const defaultLayout = [
        { id: 'header', enabled: true },
        { id: 'customer', enabled: true },
        { id: 'table', enabled: true },
        { id: 'notes', enabled: true },
        { id: 'signatures', enabled: true },
        { id: 'footer', enabled: true }
    ];

    const activeLayout = layout || defaultLayout;

    const renderSection = (sectionId) => {
        switch (sectionId) {
            case 'header':
                return (
                    <div className="pdf-header">
                        <div className="header-left">
                            {config.showLogo && (
                                <div className="company-logo">
                                    {companyData.logo ? (
                                        <img src={companyData.logo} alt="Logo" />
                                    ) : (
                                        <div>LOGO</div>
                                    )}
                                </div>
                            )}
                            <div className="company-info">
                                <div className="company-name">{companyData.name}</div>
                                <div className="company-details header-info-grid">
                                    <div className="header-info-line">
                                        <span className="header-info-label">{t.phone}:</span>
                                        <span>{companyData.phone}</span>
                                    </div>
                                    <div className="header-info-line">
                                        <span className="header-info-label">{t.address}:</span>
                                        <span>{companyData.address}</span>
                                    </div>
                                    <div className="header-info-line">
                                        <span className="header-info-label">{t.email}:</span>
                                        <span>{companyData.email}</span>
                                    </div>
                                    <div className="header-info-line">
                                        <span className="header-info-label">Web:</span>
                                        <span>{companyData.website || '-'}</span>
                                    </div>
                                    <div className="header-info-line">
                                        <span className="header-info-label">{t.authorized}:</span>
                                        <span>{companyData.authorized}</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="quote-info" style={{ borderColor: color }}>
                            <div className="quote-title">{config.title}</div>
                            <div className="quote-meta">
                                <div><strong>{t.quoteNo}:</strong> <span className="quote-number">{quoteData.number}</span></div>
                                <div><strong>{t.date}:</strong> {formatDate(quoteData.date, currentLocale)}</div>
                                <div><strong>{t.validUntil}:</strong> {formatDate(quoteData.validUntil, currentLocale)}</div>
                                <div><strong>{t.payment}:</strong> {quoteData.terms || '-'}</div>
                            </div>
                        </div>
                    </div>
                );

            case 'customer':
                return (
                    <div className="customer-section">
                        <div className="customer-box">
                            <div className="section-title">
                                <i className="fas fa-building"></i> SATICI
                            </div>
                            <div className="info-grid">
                                <div className="info-line">
                                    <span className="info-label">Firma:</span>
                                    <span className="info-value">{companyData.name}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">Yetkili:</span>
                                    <span className="info-value">{companyData.authorized}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">Tel:</span>
                                    <span className="info-value">{companyData.phone}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">E-posta:</span>
                                    <span className="info-value">{companyData.email}</span>
                                </div>
                            </div>
                        </div>

                        <div className="customer-box">
                            <div className="section-title">
                                <i className="fas fa-user"></i> {t.customer}
                            </div>
                            <div className="info-grid">
                                <div className="info-line">
                                    <span className="info-label">{t.company}:</span>
                                    <span className="info-value">{customerData.company}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">{t.authorized}:</span>
                                    <span className="info-value">{customerData.name}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">{t.phone}:</span>
                                    <span className="info-value">{customerData.phone}</span>
                                </div>
                                <div className="info-line">
                                    <span className="info-label">{t.email}:</span>
                                    <span className="info-value">{customerData.email}</span>
                                </div>
                                {customerData.taxOffice && (
                                    <div className="info-line">
                                        <span className="info-label">{t.tax}:</span>
                                        <span className="info-value">{customerData.taxOffice} / {customerData.taxNo}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    </div>
                );

            case 'table':
                return (
                    <>
                        <table className="pdf-items-table">
                            <thead>
                                <tr>
                                    <th width="25" style={{ textAlign: 'center' }}>#</th>
                                    {config.showTableImages && <th width="45" style={{ textAlign: 'center' }}>{t.image}</th>}
                                    <th style={{ textAlign: 'left' }}>{t.item} / {t.description}</th>
                                    {config.showTableUnit && <th width="60" style={{ textAlign: 'center' }}>{t.unit}</th>}
                                    <th width="60" style={{ textAlign: 'center' }}>{t.quantity}</th>
                                    <th width="90" style={{ textAlign: 'center' }}>{t.unitPrice}</th>
                                    {hasLineItemDiscounts && <th width="60" style={{ textAlign: 'center' }}>{t.discount}</th>}
                                    {config.showTableTax && <th width="50" style={{ textAlign: 'center' }}>{t.vat}</th>}
                                    <th width="90" style={{ textAlign: 'center' }}>{t.total}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.map((item, index) => (
                                    <tr key={index}>
                                        <td style={{ textAlign: 'center' }}>{index + 1}</td>
                                        {config.showTableImages && (
                                            <td style={{ textAlign: 'center' }}>
                                                <div className="item-image">
                                                    {item.image ? (
                                                        <img src={item.image} alt="" />
                                                    ) : (
                                                        '-'
                                                    )}
                                                </div>
                                            </td>
                                        )}
                                        <td>
                                            <div className="item-description">{item.name}</div>
                                            {item.description && (
                                                <div className="item-details">
                                                    {item.description}
                                                </div>
                                            )}
                                        </td>
                                        {config.showTableUnit && <td className="item-unit">{item.unit}</td>}
                                        <td className="item-quantity">{item.quantity}</td>
                                        <td className="item-price">{formatCurrency(item.price)}</td>
                                        {hasLineItemDiscounts && <td className="item-discount" style={{ textAlign: 'center', color: '#ef4444' }}>%{item.discountRate || 0}</td>}
                                        {config.showTableTax && <td className="item-tax">%{item.taxRate}</td>}
                                        <td className="item-total">
                                            {formatCurrency((item.quantity || 0) * (item.price || 0) * (1 - (item.discountRate || 0) / 100))}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>

                        <div className="pdf-summary">
                            {config.showSummary && (
                                <div className="summary-box">
                                    <div className="summary-title">
                                        <i className="fas fa-calculator"></i> {t.summary}
                                    </div>
                                    <div className="summary-row">
                                        <span className="text-slate-500">{t.subtotal}:</span>
                                        <span className="font-medium text-slate-900">{formatCurrency(subtotal)}</span>
                                    </div>
                                    {discountAmount > 0 && (
                                        <div className="summary-row">
                                            <span className="text-red-500">{t.discount} {discount?.type === 'percentage' ? `(% ${discount.value})` : ''}:</span>
                                            <span className="font-medium text-red-500">-{formatCurrency(discountAmount)}</span>
                                        </div>
                                    )}
                                    {config.showTableTax && (
                                        <>
                                            {Object.entries(taxBreakdown).map(([rate, amount]) => (
                                                <div key={rate} className="summary-row">
                                                    <span className="text-slate-500">{t.vat} (%{rate}):</span>
                                                    <span className="font-medium text-slate-900">{formatCurrency(amount)}</span>
                                                </div>
                                            ))}
                                            {Object.keys(taxBreakdown).length === 0 && totalTax > 0 && (
                                                <div className="summary-row">
                                                    <span className="text-slate-500">{t.vat}:</span>
                                                    <span className="font-medium text-slate-900">{formatCurrency(totalTax)}</span>
                                                </div>
                                            )}
                                            <div className="summary-row">
                                                <span className="font-semibold text-slate-700">{t.total} {t.vat}:</span>
                                                <span className="font-semibold text-slate-900">{formatCurrency(totalTax)}</span>
                                            </div>
                                        </>
                                    )}
                                    <div className="summary-row grand-total">
                                        <span>{t.generalTotal}:</span>
                                        <span>{formatCurrency(total)}</span>
                                    </div>
                                </div>
                            )}

                            <div>
                                {config.showBankInfo && (
                                    <div className="summary-box" style={{ marginBottom: config.showNotes ? '1rem' : 0 }}>
                                        <div className="summary-title">
                                            {t.bankInfo}
                                        </div>
                                        <div className="info-grid">
                                            <div className="info-line">
                                                <span className="info-label">{t.bank}:</span>
                                                <span className="info-value">{bankData.bankName || '-'}</span>
                                            </div>
                                            <div className="info-line">
                                                <span className="info-label">{t.branch}:</span>
                                                <span className="info-value">{bankData.branch || '-'}</span>
                                            </div>
                                            <div className="info-line">
                                                <span className="info-label">{t.accountNo}:</span>
                                                <span className="info-value">{bankData.accountNo || '-'}</span>
                                            </div>
                                            <div className="info-line">
                                                <span className="info-label">{t.iban}:</span>
                                                <span className="info-value" style={{ fontFamily: 'monospace' }}>{bankData.iban || '-'}</span>
                                            </div>
                                            <div className="info-line">
                                                <span className="info-label">{t.accountHolder}:</span>
                                                <span className="info-value">{bankData.accountHolder || '-'}</span>
                                            </div>
                                        </div>
                                    </div>
                                )}
                            </div>
                        </div>
                    </>
                );

            case 'notes':
                return (
                    <>
                        {config.showNotes && quoteData.notes && (
                            <div className="summary-box" style={{ marginTop: '1rem', marginBottom: '1rem' }}>
                                <div className="summary-title">{t.notes}</div>
                                <div style={{ color: '#475569', lineHeight: 1.3, fontSize: '0.7rem' }}>{quoteData.notes}</div>
                            </div>
                        )}
                        {config.showTerms && (
                            <div className="terms-section">
                                <div className="terms-grid">
                                    {(quoteData.validUntil || quoteData.terms) && (
                                        <div className="term-card">
                                            <h3>{t.terms}</h3>
                                            <div className="term-content">
                                                {quoteData.validUntil && (
                                                    <div className="mb-1">
                                                        <strong>{t.validUntil}:</strong> {formatDate(quoteData.validUntil, currentLocale)} {t.validUntilDate}.
                                                    </div>
                                                )}
                                                {quoteData.terms && (
                                                    <div>
                                                        <strong>{t.payment}:</strong> {quoteData.terms}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                    {(quoteData.deliveryTerms || quoteData.warrantyTerms) && (
                                        <div className="term-card">
                                            <h3>{t.deliveryWarranty}</h3>
                                            <div className="term-content">
                                                {quoteData.deliveryTerms && (
                                                    <div className="mb-1">
                                                        <strong>{t.delivery}:</strong> {quoteData.deliveryTerms}
                                                    </div>
                                                )}
                                                {quoteData.warrantyTerms && (
                                                    <div>
                                                        <strong>{t.warranty}:</strong> {quoteData.warrantyTerms}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                </div>
                            </div>
                        )}
                    </>
                );

            case 'signatures':
                return config.showSignatures ? (
                    <div className="signature-section">
                        <div className="signature-box">
                            <div className="signature-area">
                                {signature ? (
                                    <img src={signature} alt="Dijital İmza" style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                                ) : companyData.signature ? (
                                    <img src={companyData.signature} alt="İmza" style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                                ) : (
                                    <span style={{ color: '#cbd5e1', fontSize: '0.7rem' }}>{t.signature}</span>
                                )}
                            </div>
                            <div className="signature-label">{t.deliveredBy}</div>
                        </div>
                        <div className="signature-box">
                            <div className="stamp-area" style={companyData.stamp ? { border: 'none', background: 'transparent' } : {}}>
                                {companyData.stamp ? (
                                    <img src={companyData.stamp} alt="Kaşe" style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                                ) : (
                                    <span style={{ color: '#cbd5e1', fontSize: '0.7rem' }}>{t.stamp} / {t.signature}</span>
                                )}
                            </div>
                            <div className="signature-label">{t.receivedBy}</div>
                        </div>
                    </div>
                ) : null;

            case 'footer':
                return (
                    <>
                        <div className="pdf-footer">
                            <div className="footer-contact">
                                <div>{companyData.address}</div>
                                <div>{companyData.phone} | {companyData.email}</div>
                                <div>{companyData.website}</div>
                            </div>
                            <div className="footer-thanks">
                            </div>
                        </div>
                        {config.customFooter && (
                            <div style={{
                                marginTop: '1rem',
                                paddingTop: '0.5rem',
                                borderTop: '1px solid #e2e8f0',
                                textAlign: 'center',
                                fontSize: '0.65rem',
                                color: '#94a3b8'
                            }}>
                                {config.customFooter}
                            </div>
                        )}
                    </>
                );

            default:
                return null;
        }
    };

    // Modern Theme Render
    if (theme === 'modern') {
        return (
            <>
                <div id={id} className="modern-theme-container w-full max-w-[210mm] mx-auto" style={containerStyles}>
                    <style>{modernStyles}</style>

                    {/* Watermark */}
                    {config.showWatermark && (
                        <div
                            style={{
                                position: 'absolute',
                                inset: 0,
                                display: 'flex',
                                alignItems: 'center',
                                justifyContent: 'center',
                                pointerEvents: 'none',
                                zIndex: 0,
                                transform: `rotate(${config.watermarkRotation || -45}deg)`,
                                opacity: config.watermarkOpacity,
                                fontSize: `${config.watermarkFontSize || 120}px`,
                                fontWeight: 'bold',
                                color: config.watermarkColor || '#000000',
                                whiteSpace: 'nowrap'
                            }}
                        >
                            {config.watermarkText}
                        </div>
                    )}

                    {activeLayout.map(section => {
                        if (!section.enabled) return null;
                        return (
                            <React.Fragment key={section.id}>
                                {renderSection(section.id)}
                            </React.Fragment>
                        );
                    })}
                </div>
            </>
        );
    }

    // Classic Theme Render
    if (theme === 'classic') {
        return (
            <div id={id} className="classic-theme-container w-full max-w-[210mm] mx-auto" style={containerStyles}>
                <style>{modernStyles}</style>
                <div className="pdf-header" style={{ borderBottom: '2px solid #000', paddingBottom: '1rem', marginBottom: '2rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div className="company-info">
                            <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>{companyData.name}</h1>
                            <div style={{ fontSize: '0.8rem' }}>
                                <div>{companyData.address}</div>
                                <div>{companyData.phone} | {companyData.email}</div>
                                <div>{companyData.website}</div>
                            </div>
                        </div>
                        {config.showLogo && companyData.logo && (
                            <img src={companyData.logo} alt="Logo" style={{ height: '60px', objectFit: 'contain' }} />
                        )}
                    </div>
                </div>

                <div className="quote-details" style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between' }}>
                    <div className="customer-info">
                        <h3 style={{ borderBottom: '1px solid #ccc', paddingBottom: '0.25rem', marginBottom: '0.5rem' }}>{t.customer}</h3>
                        <div><strong>{customerData.company}</strong></div>
                        <div>{customerData.name}</div>
                        <div>{customerData.phone}</div>
                        <div>{customerData.email}</div>
                    </div>
                    <div className="quote-meta" style={{ textAlign: 'right' }}>
                        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>{config.title}</h2>
                        <div><strong>{t.quoteNo}:</strong> {quoteData.number}</div>
                        <div><strong>{t.date}:</strong> {formatDate(quoteData.date, currentLocale)}</div>
                        <div><strong>{t.validUntil}:</strong> {formatDate(quoteData.validUntil, currentLocale)}</div>
                    </div>
                </div>

                <table className="pdf-items-table" style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2rem' }}>
                    <thead>
                        <tr style={{ borderBottom: '2px solid #000' }}>
                            <th style={{ textAlign: 'left', padding: '0.5rem' }}>#</th>
                            <th style={{ textAlign: 'left', padding: '0.5rem' }}>{t.item}</th>
                            <th style={{ textAlign: 'center', padding: '0.5rem' }}>{t.quantity}</th>
                            <th style={{ textAlign: 'right', padding: '0.5rem' }}>{t.unitPrice}</th>
                            <th style={{ textAlign: 'right', padding: '0.5rem' }}>{t.total}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((item, index) => (
                            <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                                <td style={{ padding: '0.5rem' }}>{index + 1}</td>
                                <td style={{ padding: '0.5rem' }}>
                                    <div style={{ fontWeight: 'bold' }}>{item.name}</div>
                                    <div style={{ fontSize: '0.8rem', color: '#666' }}>{item.description}</div>
                                </td>
                                <td style={{ textAlign: 'center', padding: '0.5rem' }}>{item.quantity} {item.unit}</td>
                                <td style={{ textAlign: 'right', padding: '0.5rem' }}>{formatCurrency(item.price)}</td>
                                <td style={{ textAlign: 'right', padding: '0.5rem' }}>{formatCurrency(item.total)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>

                <div className="summary-section" style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <div style={{ width: '300px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                            <span>{t.subtotal}:</span>
                            <span>{formatCurrency(subtotal)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', fontWeight: 'bold', fontSize: '1.1rem', borderTop: '1px solid #000', paddingTop: '0.5rem' }}>
                            <span>{t.generalTotal}:</span>
                            <span>{formatCurrency(total)}</span>
                        </div>
                    </div>
                </div>
            </div>
        );
    }

    // Minimal Theme Render
    if (theme === 'minimal') {
        return (
            <div id={id} className="minimal-theme-container w-full max-w-[210mm] mx-auto" style={containerStyles}>
                <style>{modernStyles}</style>
                <div style={{ marginBottom: '3rem' }}>
                    <h1 style={{ fontSize: '2rem', fontWeight: '300', marginBottom: '1rem' }}>{config.title}</h1>
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: '#666' }}>
                        <div>
                            <div>{t.quoteNo}: {quoteData.number}</div>
                            <div>{t.date}: {formatDate(quoteData.date, currentLocale)}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                            <div>{companyData.name}</div>
                            <div>{companyData.email}</div>
                        </div>
                    </div>
                </div>

                <table className="pdf-items-table" style={{ width: '100%', marginBottom: '2rem' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid #eee', color: '#999' }}>
                            <th style={{ textAlign: 'left', padding: '1rem 0', fontWeight: 'normal' }}>{t.item}</th>
                            <th style={{ textAlign: 'center', padding: '1rem 0', fontWeight: 'normal' }}>{t.quantity}</th>
                            <th style={{ textAlign: 'right', padding: '1rem 0', fontWeight: 'normal' }}>{t.unitPrice}</th>
                            <th style={{ textAlign: 'right', padding: '1rem 0', fontWeight: 'normal' }}>{t.total}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.map((item, index) => (
                            <tr key={index} style={{ borderBottom: '1px solid #f5f5f5' }}>
                                <td style={{ padding: '1rem 0' }}>{item.name}</td>
                                <td style={{ textAlign: 'center', padding: '1rem 0' }}>{item.quantity}</td>
                                <td style={{ textAlign: 'right', padding: '1rem 0' }}>{formatCurrency(item.price)}</td>
                                <td style={{ textAlign: 'right', padding: '1rem 0' }}>{formatCurrency(item.total)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>

                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '2rem' }}>
                    <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '1.5rem', fontWeight: '300' }}>{formatCurrency(total)}</div>
                        <div style={{ color: '#999', fontSize: '0.8rem' }}>{t.generalTotal}</div>
                    </div>
                </div>
            </div>
        );
    }

    // Corporate Theme Render
    if (theme === 'corporate') {
        return (
            <div id={id} className="corporate-theme-container w-full max-w-[210mm] mx-auto" style={containerStyles}>
                <style>{modernStyles}</style>
                <div style={{ background: color, color: 'white', padding: '2rem', marginBottom: '2rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <h1 style={{ fontSize: '1.8rem', margin: 0 }}>{config.title}</h1>
                            <div style={{ opacity: 0.8, marginTop: '0.5rem' }}>#{quoteData.number}</div>
                        </div>
                        {config.showLogo && companyData.logo && (
                            <div style={{ background: 'white', padding: '0.5rem', borderRadius: '4px' }}>
                                <img src={companyData.logo} alt="Logo" style={{ height: '50px', objectFit: 'contain' }} />
                            </div>
                        )}
                    </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', padding: '0 2rem', marginBottom: '2rem' }}>
                    <div>
                        <h3 style={{ color: color, borderBottom: `1px solid ${color} `, paddingBottom: '0.5rem', marginBottom: '1rem' }}>{t.customer}</h3>
                        <div><strong>{customerData.company}</strong></div>
                        <div>{customerData.name}</div>
                    </div>
                    <div>
                        <h3 style={{ color: color, borderBottom: `1px solid ${color} `, paddingBottom: '0.5rem', marginBottom: '1rem' }}>{t.company}</h3>
                        <div><strong>{companyData.name}</strong></div>
                        <div>{companyData.email}</div>
                    </div>
                </div>

                <div style={{ padding: '0 2rem' }}>
                    <table className="pdf-items-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: '#f8fafc' }}>
                                <th style={{ padding: '1rem', textAlign: 'left' }}>{t.item}</th>
                                <th style={{ padding: '1rem', textAlign: 'center' }}>{t.quantity}</th>
                                <th style={{ padding: '1rem', textAlign: 'right' }}>{t.unitPrice}</th>
                                <th style={{ padding: '1rem', textAlign: 'right' }}>{t.total}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {items.map((item, index) => (
                                <tr key={index} style={{ borderBottom: '1px solid #e2e8f0' }}>
                                    <td style={{ padding: '1rem' }}>{item.name}</td>
                                    <td style={{ padding: '1rem', textAlign: 'center' }}>{item.quantity}</td>
                                    <td style={{ padding: '1rem', textAlign: 'right' }}>{formatCurrency(item.price)}</td>
                                    <td style={{ padding: '1rem', textAlign: 'right' }}>{formatCurrency(item.total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>

                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '2rem' }}>
                        <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '8px', width: '300px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                <span>{t.subtotal}:</span>
                                <span>{formatCurrency(subtotal)}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: '1.1rem', color: color }}>
                                <span>{t.generalTotal}:</span>
                                <span>{formatCurrency(total)}</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        );
    }

    return null;
};

export default PrintableQuote;

