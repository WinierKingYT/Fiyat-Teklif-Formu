            ...
            <div className="flex flex-col h-[70vh]">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <div className="flex gap-2">
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'all' ? 'bg-[var(--primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] hover:bg-[var(--bg-hover)]'}`}
                            onClick={() => setActiveTab('all')}
                        >
                            Tümü
                        </button>
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'customers' ? 'bg-[var(--primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] hover:bg-[var(--bg-hover)]'}`}
                            onClick={() => setActiveTab('customers')}
                        >
                            Müşteriler
                        </button>
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'products' ? 'bg-[var(--primary)] text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] hover:bg-[var(--bg-hover)]'}`}
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-color)] p-2">
                    {filteredItems.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {filteredItems.map(item => (
                                <div key={item.id} className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-[var(--border-color)] flex justify-between items-center hover:shadow-sm transition-shadow">
